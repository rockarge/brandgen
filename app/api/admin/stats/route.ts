...
export async function GET(req: NextRequest) {
  ...
  const { data: credits, error: creditsErr } = await db.from("credits")
  ...
  const totalCredits = creditsData.reduce((s, c) => s + (c.balance ?? 0), 0);
  return NextResponse.json({ stats: { totalJobs, paidJobs, doneJobs, errorJobs, pendingJobs, tierCounts, totalCreditsPurchased: creditsData.length, totalCreditsRemaining: totalCredits }, jobs: jobs ?? [], credits: creditsData });
}